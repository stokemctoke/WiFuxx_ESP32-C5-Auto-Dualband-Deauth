...
build: one binary, `WiFuxx_v2.2.0_merged.bin`, runs on both boards.
...
The DevKit uses the **identical** `WiFuxx_v2.2.0_merged.bin` as the XIAO build — the
...
1. Download `WiFuxx_v2.2.0_merged.bin` from the [Releases](https://github.com/stokemctoke/WiFuxx_ESP32-C5-Auto-Dualband-Deauth/releases) page.
...
4. Choose **Custom Flash** and select `WiFuxx_v2.2.0_merged.bin`.
...
```bash
esptool.py -p /dev/ttyACM0 write_flash 0x0 WiFuxx_v2.2.0_merged.bin
...
| --- | --- | --- |
| Binary | `WiFuxx_v2.2.0_merged.bin` | **same** `WiFuxx_v2.2.0_merged.bin` |
